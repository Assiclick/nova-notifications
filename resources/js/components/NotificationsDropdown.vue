...
export default {
  ...
  mounted() {
    ...
    self.loadNotifications(function(response) {
      Echo.private(
        Nova.config.user_model_namespace + "." + response.data.user_id
  ...
  methods: {
    loadNotifications: function(callback) {
      axios.get("/nova-vendor/nova-notifications/unread").then((response) => {
    ...
    markAllAsRead: function() {
      axios.patch("/nova-vendor/nova-notifications").then(() => {
        this.notifications = [];
        this.count = 0;
      });
    },
